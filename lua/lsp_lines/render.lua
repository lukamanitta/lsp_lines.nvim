        ...
        for i = #lelements, 1, -1 do -- last element goes on top
            if lelements[i][1] == DIAGNOSTIC then
                ...
                if overlap and multi > 0 then
                    center_symbol = vim.opt.fillchars:get().verthoriz
                elseif overlap then
                    center_symbol = vim.opt.fillchars:get().vertright
                elseif multi > 0 then
                ...
                local center = {
                    {
                        string.format(
                            "%s%s%s",
                            center_symbol,
                            string.rep(vim.opt.fillchars:get().horiz, 4),
                            " "
                        ),
                        highlight_groups[diagnostic.severity],
                    },
